...
function hdlSubmit(e){
  ...
  return (
    ...
    <form className="w-full bg-red-300 p-4 border-2 border-sky-500" onSubmit={hdlSubmit}>
    ...
    {checkList.finalList.map((el,index)=><div className="flex justify-between shadow-md border border-black p-1.5"><p>{el}</p><p>${checkList.finalPrice[index].toFixed(2)}</p></div>)}
    {checkList.total==0?<></>:<div><p>Total</p><p>{checkList.total}</p></div>}
    </div>
    </div>
